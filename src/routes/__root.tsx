        ...
        <aside className="w-64 flex-shrink-0 flex flex-col border-r border-slate-800/50">
          ...
          <nav className="flex-1 py-4 overflow-y-auto">
            ...
            <div className="px-3 space-y-1 mb-6">
              {[
                { label: 'Daily Logs', to: '/daily-logs' },
                { label: 'Daily Rounds', to: '#' },
                { label: 'Tasks', to: '#' },
                { label: 'Feeding Schedule', to: '#' }
              ].map(item => (
                <Link key={item.label} to={item.to} activeProps={{ className: "bg-slate-800 text-white font-bold" }} className="flex items-center gap-3 px-3 py-2 text-slate-300 hover:text-white rounded hover:bg-slate-800/50 cursor-pointer">
